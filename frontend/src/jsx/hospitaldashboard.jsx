...
const HospitalDashboard = () => {
  ...
  const fetchDashboardData = async () => {
    try {
      ...
      // Fetch appointments
      const appointmentsRes = await axios.get(`${import.meta.env.VITE_API_URL || 'https://blood-bank-backend.onrender.com'}/hospital/my-appointments`, {
        headers: { Authorization: `Bearer ${token}` }
      });
      setAppointments(appointmentsRes.data);

      // Fetch blood stock
      const stockRes = await axios.get(`${import.meta.env.VITE_API_URL || 'https://blood-bank-backend.onrender.com'}/hospital/stock`, {
        headers: { Authorization: `Bearer ${token}` }
      });
      setBloodStock(stockRes.data);

      // Fetch plasma stock
      const plasmaRes = await axios.get(`${import.meta.env.VITE_API_URL || 'https://blood-bank-backend.onrender.com'}/hospital/plasma-stock`, {
        headers: { Authorization: `Bearer ${token}` }
      });
  ...
  const handleStatusUpdate = async (appointmentId, newStatus, notes = '') => {
    ...
    try {
      ...
      const response = await axios.patch(`${import.meta.env.VITE_API_URL || 'https://blood-bank-backend.onrender.com'}/hospital/appointments/${appointmentId}/status`, {
        status: newStatus,
        notes,
