...
const HospitalStaffDashboard = () => {
  ...
  const fetchDashboardData = async () => {
    try {
      ...
      const appointmentsRes = await axios.get(`${import.meta.env.VITE_API_URL || 'https://blood-bank-backend.onrender.com'}/hospital-staff/appointments/upcoming/${userData.id}`, {
        headers: { Authorization: `Bearer ${userData.token}` }
      });
      setAppointments(appointmentsRes.data);

      // Fetch blood stock
      const stockRes = await axios.get(`${import.meta.env.VITE_API_URL || 'https://blood-bank-backend.onrender.com'}/staff/stock`, {
        headers: { Authorization: `Bearer ${userData.token}` }
      });
      setBloodStock(stockRes.data);

      // Fetch notifications
      const notificationsRes = await axios.get(`${import.meta.env.VITE_API_URL || 'https://blood-bank-backend.onrender.com'}/staff/notifications`, {
        headers: { Authorization: `Bearer ${userData.token}` }
      });
  ...
  const handleMarkDonated = async (appointmentId) => {
    try {
      const userData = JSON.parse(localStorage.getItem('user'));
      await axios.patch(`${import.meta.env.VITE_API_URL || 'https://blood-bank-backend.onrender.com'}/staff/appointments/${appointmentId}/confirm`, {
        staffId: userData.id,
        status: 'donated',
        notes: ''
      }, {
  ...
  const handleStockAdjustment = async (bloodGroup, adjustment) => {
    try {
      const userData = JSON.parse(localStorage.getItem('user'));
      await axios.post(`${import.meta.env.VITE_API_URL || 'https://blood-bank-backend.onrender.com'}/staff/stock/adjust`, {
        bloodGroup,
        adjustment,
